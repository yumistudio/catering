...
function yumi_setup() {
	...
	add_image_size( 'yumi-video-thumbnal', 370, 280, true );
	add_image_size( 'yumi-slider-image', 270, 270, true );
...
function yumi_scripts() {
	...
	wp_enqueue_script( 'isotope', get_theme_file_uri( '/assets/js/isotope/isotope.pkgd.min.js' ), array(), '', false );
...
function prefix_movie_rewrite_rule() {
    add_rewrite_rule( 'galeria/foto', 'index.php?post_type=tribe_events&gallery_mode=photo', 'top' );
    add_rewrite_rule( 'galeria/video', 'index.php?post_type=tribe_events&gallery_mode=video', 'top' );
    add_rewrite_rule( 'wydarzenie/([^/]+)/foto', 'index.php?tribe_events=$matches[1]&view=photo', 'top' );
}
...
function prefix_url_rewrite_templates() {
    ...
    if ( get_query_var( 'gallery_mode' ) == 'photo' && !is_singular( 'tribe_events' ) ) {
        add_filter( 'template_include', function() {
            return get_template_directory() . '/page-templates/event-gallery-image.php';
        });
    } elseif ( get_query_var( 'view' ) == 'photo' && is_singular( 'tribe_events' ) ) {
        add_filter( 'template_include', function() {
            return get_template_directory() . '/page-templates/event-gallery-single.php';
        });
    } elseif ( get_query_var( 'gallery_mode' ) == 'video' && !is_singular( 'tribe_events' ) ) {
        add_filter( 'template_include', function() {
            return get_template_directory() . '/page-templates/event-gallery-video.php';
        });
    }
}

function yumi_only_galleries( $query ) {
    /*
    if (false && get_query_var( 'gallery_mode' ) == 'photo'&& $query->is_main_query() ) {
        $meta_query = array(
            array(
				'key' => 'image_gallery',
				'value' => array(''),
...
add_action( 'pre_get_posts', 'yumi_only_galleries' );
...
?>
    ...
    <style>#event_tribe_venue.eventtable { display: none !important;}</style>
    <style>#event_cost.eventtable { display: none !important;}</style>
    <style>#event_cost.eventtable { display: none !important;}</style>
