...
function twentyseventeen_dropdown_icon_to_menu_link( $title, $item, $args, $depth ) {
	if ( 'top' === $args->theme_location ) {
		...
		if ($depth === 1 && intval($item->object_id) && $icon = get_field('page_icon', $item->object_id)) {
			$title = '<svg class="icon" viewBox="0 0 30 30" class="icon">'.
						'<use xlink:href="'.$icon.'#Layer_1"></use>'.
					  '</svg>'. $title;
		}
